#!/usr/bin/env bash
# build.sh — validate and pack the Apple Contacts MCP desktop extension
set -euo pipefail
...
INIT_VERSION=$(sed -n 's/^__version__ = "\(.*\)"$/\1/p' "${SCRIPT_DIR}/src/apple_contacts_mcp/__init__.py")
...
src = open(f"{root}/src/apple_contacts_mcp/server.py").read()
...
mkdir -p "${OUT}"
STABLE="${OUT}/apple-contacts.mcpb"
VERSIONED="${OUT}/apple-contacts-${VERSION}.mcpb"
...
if [[ -e "${VERSIONED}" && "${FORCE}" -ne 1 ]]; then
    ...
    echo "    pyproject.toml, manifest.json, src/apple_contacts_mcp/__init__.py" >&2
    echo "" >&2
...
for F in "${STABLE}" "${VERSIONED}"; do
    osascript -e "tell application \"Finder\" to set comment of (POSIX file \"${F}\" as alias) to \"Apple Contacts MCP — v${VERSION}\"" >/dev/null 2>&1 || true
...
# it survives. The moving `apple-contacts.mcpb` copy is deliberately left
...
if [[ ! -e "${LEDGER}" ]]; then
    {
        echo "# SHA-1 checksums of every Apple Contacts MCP bundle built from this repo."
        echo "#"
...
echo ""
echo "ℹ  macOS will prompt for Contacts access on first use. You can also pre-grant"
echo "   under System Settings → Privacy & Security → Contacts."
